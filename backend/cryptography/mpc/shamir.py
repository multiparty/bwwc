...
class SecretShare(object):
    ...
    def sample_shamir_polynomial(self, zero_value: int) -> List[int]:
        coefs = [zero_value] + [random.randrange(self.prime) for _ in range(self.threshold - 1)]
        return coefs
    ...
    def interpolate_at_point(self, points_values: List[Tuple[int, int]], point: int) -> int:
    ...
    def shamir_share(self, secret: int, num_shares: int) -> List[Tuple[int, int]]:
        ...
        shares = [(i, self.evaluate_at_point(polynomial, i)) for i in range(1, num_shares + 1)]
        return shares
    ...
    def shamir_add(self, x, y):
        return [ (i+1, (xi[1] + yi[1]) % self.prime) for i, (xi, yi) in enumerate(list(zip(x, y))) ]